...
iamInfoExtract <- function( master_config, iam ) {
  ...
  #if ( iam == 'AIM' ) { iam_name <<- 'AIM/CGE' } # AIM Team requested model name be "AIM"
  ...
  ds_start_year <<- tango_iam_list$downscaling_start_year 
  ds_end_year <<- tango_iam_list$downscaling_end_year
