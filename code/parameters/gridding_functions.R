...
#          step is tor and its wraper grid_all_sectors.
...
# ------------------------------------------------------------------------------
# add_seasonality
# Brief: add seasoanlity to annual flux grids; different strategies are adapted for different sectors
# Dependencies:
# Author: Leyang Feng
...
add_seasonality <- function( annual_flux, em, sector, year, days_in_month, grid_resolution, seasonality_mapping ) {
  ...
  file_name <- seasonality_mapping[ seasonality_mapping$em == em &
                                      seasonality_mapping$sector == sector &
                                      seasonality_mapping$year == year, 'seasonality_file' ]
  stopifnot(length(file_name) > 0)
  if ( file_name %in% common_seasonality_list ) { # common_seasonality_list exists in the global environment
